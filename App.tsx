...
const App: React.FC = () => {
    ...
    if (storedUser && token) {
      try {
        ...
        const savedAvatar = localStorage.getItem(
          `avatar_${parsedUser._id || parsedUser.id}`
        );
  ...
  // Load medicines when user or view changes
  useEffect(() => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuthSuccess(
        res.message || 'If an account matches, a reset link has been sent.'
      );
      setFpEmail('');
    } catch (err: any) {
  ...
  const handleSaveMedicine = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        // Update
        const updatedMed = await api.updateMedicine(token, editingId, {
          name: medName,
          dosage: medDosage,
        ...
        setMedicines((prev) =>
          prev.map((m) => (m._id === editingId ? updatedMed : m))
        );
        handleCancelEdit();
      } else {
        // Create
        const newMed = await api.addMedicine(token, {
          name: medName,
          dosage: medDosage,
          frequency: medFrequency,
          type: medType,
  ...
  const handleExportReport = () => {
    ...
    const csvContent =
      ...
      medicines
        .map(
          (m) =>
            `"${m.name}","${m.type || 'Tablet'}","${m.dosage}","${m.frequency}"`
        )
        .join('\n');
  ...
  const DarkModeToggle = ({ className }: { className?: string }) => (
    <button
      ...
      className={`p-2 rounded-full transition-colors ${
        className ??
        'text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-yellow-300'
      }`}
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleLogin}>
              ...
              <div className="flex justify-end mb-4">
                <button
                  ...
                  className="text-sm text-primary hover:underline dark:text-purple-400"
                >
                  Forgot password?
                </button>
              </div>
            ...
            <p className="mt-6 text-center text-sm text-slate-600 dark:text-slate-400">
              ...
              <button
                ...
                className="text-primary dark:text-purple-400 font-medium hover:underline"
              >
                Create one
              </button>
            </p>
          ...
          <div className="flex justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <span className="text-slate-700 dark:text-slate-200 font-medium hidden sm:block">
                ...
                <div className="mb-8 bg-gradient-to-r from-primary to-accent rounded-2xl p-8 text-white shadow-lg relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <h1 className="text-3xl font-bold mb-2 flex items-center">
                      <i
                        className={`fa-solid ${greeting.icon} mr-3 opacity-90`}
                      ></i>
                      {greeting.text},{' '}
                      {user?.name || user?.username?.split(' ')[0]}!
                    </h1>
                    <p className="text-purple-100 text-lg italic opacity-90 mb-6">
                      "{dailyQuote}"
                    </p>
              ...
              <div className="lg:col-span-1 space-y-6">
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-6 transition-colors">
                  <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-4 flex items-center">
                    <i className="fa-solid fa-chart-pie text-primary mr-2"></i>{' '}
                    Overview
                  </h3>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-purple-50 dark:bg-purple-900/30 p-4 rounded-lg text-center">
                      <span className="block text-2xl font-bold text-primary dark:text-purple-400">
                        {medicines.length}
                      </span>
                    ...
                    <div className="bg-pink-50 dark:bg-pink-900/30 p-4 rounded-lg text-center">
                      <span className="block text-2xl font-bold text-pink-500 dark:text-pink-400">
                        {
                          medicines.filter(
                            (m) => m.type === 'Tablet' || !m.type
                          ).length
                        }
                      </span>
                    ...
                    <div className="flex items-center">
                      <div className="bg-purple-100 dark:bg-purple-900 p-2 rounded-lg mr-3">
                        <i
                          ...
                          } text-primary dark:text-purple-300 text-sm`}
                        ></i>
                      </div>
                    ...
                    <div className="mb-4">
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          value={medType}
                          onChange={(e) =>
                            setMedType(e.target.value as MedicineType)
                          }
                        >
                ...
                <div className="flex flex-wrap justify-center gap-4 mb-6">
                  {AVATARS.map((char) => (
                    <button
                      ...
                      className={`text-4xl w-16 h-16 rounded-full flex items-center justify-center transition-all ${
                        avatar === char
                          ? 'bg-purple-100 dark:bg-purple-900 border-2 border-primary scale-110'
